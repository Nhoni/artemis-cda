                ...
                <tbody>
                    <?php foreach ($clients as $client) : ?>
                        <tr class="text-xs bg-gray-50">
                            <td class="py-5 px-6 font-medium"><?= !empty($client['id']) ? $client['id'] : 'Undefined'; ?></td>
                            <td class="flex px-4 py-3">
                                <img class="w-8 h-8 mr-4 object-cover rounded-md" src="https://placehold.co/400x400/216be9/FFF?text=<?= !empty($client['name']) ? (substr($client['name'], 0, 1)) : 'C' ?>" alt="">
                                <div>
                                    <p class="font-medium"><?= !empty($client['name']) ? $client['name'] : 'Undefined' ?></p>
                                    <p class="text-gray-500"><?= !empty($client['email']) ? $client['email'] : 'Undefined' ?></p>
                                </div>
                            </td>
                            <td>
                                <span class="inline-block py-1 px-2 text-white bg-<?= !empty($client['deposit']) && $client['deposit'] == 1 ? 'green' : ($client['deposit'] == 0 ? 'red' : 'gray') ?>-500 rounded-full">
                                    <?= !empty($client['deposit']) && $client['deposit'] == 1 ? 'Payé' : ($client['deposit'] == 0 ? 'Non payé' : 'Undifined') ?>
                                </span>
                            </td>
                            <td>
                                <button type="button" onclick="showModal('edit')" class="inline-block mr-2">
                                ...
                                <button type="button" onclick="showModal('delete')" class="inline-block mr-2">
